...
if (!isset($_POST['main_category_name']) || !isset($_POST['image'])) {
...
$stmt = $con->prepare("INSERT INTO main_category (main_category_name, image) VALUES (?, ?)");
...
$stmt->bind_param("sb", $main_category_name, $image);

// Bind image as long data
$stmt->send_long_data(1, $image); // The 1 corresponds to the image parameter

// Execute the query
if ($stmt->execute()) {
    echo json_encode(["success" => true, "message" => "Main category inserted successfully"]);
} else {
